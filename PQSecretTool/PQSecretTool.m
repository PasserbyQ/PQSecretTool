//
//  PQSecretTool.m
//  PQSecretTool
//
//  Created by PasserbyQ on 2018/3/9.
//  Copyright © 2018年 PasserbyQ. All rights reserved.
//

#import "PQSecretTool.h"
#import <CommonCrypto/CommonCrypto.h>

#define kKeyValue @"saSAXcasc6"

@implementation PQSecretTool

+ (NSString *)base64EncodedStringWithData:(NSData *)data
{
    if ((data == nil) || (data == NULL) || ![data isKindOfClass:[NSData class]]) {
        return nil;
    } else if (![data isKindOfClass:[NSData class]]) {
        return nil;
    }
    return [data base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];
}

+ (NSData *)base64DecodeDataWithString:(NSString *)string
{
    if ((string == nil) || (string == NULL) || ![string isKindOfClass:[NSString class]]) {
        return nil;
    }
    return [[NSData alloc] initWithBase64EncodedString:string options:NSDataBase64DecodingIgnoreUnknownCharacters];
}


+(NSString *)md5:(NSString *)str
{
    if (str == nil) {
        return @"";
    }
    const char *cStr = [str UTF8String];
    unsigned char result[16];
    CC_MD5(cStr, (CC_LONG)strlen(cStr), result); // This is the md5 call
    return [NSString stringWithFormat:
            @"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
            result[0], result[1], result[2], result[3],
            result[4], result[5], result[6], result[7],
            result[8], result[9], result[10], result[11],
            result[12], result[13], result[14], result[15]
            ];
    
}

+ (NSData *)md5WithData:(NSData *)data
{
    Byte byte[CC_MD5_DIGEST_LENGTH];
    CC_MD5((const void*)([data bytes]), (CC_LONG)[data length], byte);
    return [NSData dataWithBytes:byte length:CC_MD5_DIGEST_LENGTH];
}


+ (NSString *)encryptWithString:(NSString *)str
{
    //kCCEncrypt 加密
    return [self encrypt:str encryptOrDecrypt:kCCEncrypt key:kKeyValue];
}

+ (NSString *)decryptWithString:(NSString *)str
{
    //kCCDecrypt 解密
    return [self encrypt:str encryptOrDecrypt:kCCDecrypt key:kKeyValue];
}

+ (NSString *)encrypt:(NSString *)sText encryptOrDecrypt:(CCOperation)encryptOperation key:(NSString *)key
{
    const void *dataIn;
    size_t dataInLength;
    
    if (encryptOperation == kCCDecrypt)//解码
    {
        NSData *decryptData = [PQSecretTool base64DecodeDataWithString:sText];
        dataInLength = [decryptData length];
        dataIn = [decryptData bytes];
    }
    else//加密
    {
        NSData* encryptData = [sText dataUsingEncoding:NSUTF8StringEncoding];
        dataInLength = [encryptData length];
        dataIn = (const void *)[encryptData bytes];
    }
    NSData *keyData = [key dataUsingEncoding:NSUTF8StringEncoding];
//    NSData *keyData = [PQSecretTool base64DecodeDataWithString:sourceData];
    /*
     DES加密 ：用CCCrypt函数加密一下，然后用base64编码下，传过去
     DES解密 ：把收到的数据根据base64，decode一下，然后再用CCCrypt函数解密，得到原本的数据
     */
    CCCryptorStatus ccStatus;
    uint8_t *dataOut = NULL; //可以理解位type/typedef 的缩写（有效的维护了代码，比如：一个人用int，一个人用long。最好用typedef来定义）
    size_t dataOutAvailable = 0; //size_t  是操作符sizeof返回的结果类型
    size_t dataOutMoved = 0;
    
    dataOutAvailable = (dataInLength + kCCBlockSizeDES) & ~(kCCBlockSizeDES - 1);
    dataOut = malloc( dataOutAvailable * sizeof(uint8_t));
    memset((void *)dataOut, 0x0, dataOutAvailable);//将已开辟内存空间buffer的首 1 个字节的值设为值 0
    
    NSString *initIv = @"12345678";
    const void *iv = (const void *) [initIv UTF8String];
    //CCCrypt函数 加密/解密
    ccStatus = CCCrypt(encryptOperation,//  加密/解密
                       kCCAlgorithmDES,//  加密标准（des，3des，aes。。。。）
                       kCCOptionECBMode|kCCOptionPKCS7Padding,//  选项分组密码算法(des:对每块分组加一次密  3DES：对每块分组加三个不同的密)
                       [keyData bytes],  //密钥    加密和解密的密钥必须一致
                       kCCKeySizeDES,//   DES 密钥的大小（根据加密标准选择）
                       iv, //  可选的初始矢量
                       dataIn, // 数据的存储单元
                       dataInLength,// 数据的大小
                       (void *)dataOut,// 用于返回数据
                       dataOutAvailable,
                       &dataOutMoved);
    
    
    NSString *result = nil;
    
    if (encryptOperation == kCCDecrypt)//encryptOperation==1  解码
    {
        //得到解密出来的data数据，改变为utf-8的字符串
        result = [[NSString alloc] initWithData:[NSData dataWithBytes:(const void *)dataOut length:(NSUInteger)dataOutMoved] encoding:NSUTF8StringEncoding];
    }
    else //encryptOperation==0  （加密过程中，把加好密的数据转成base64的）
    {
        //编码 base64
        NSData *data = [NSData dataWithBytes:(const void *)dataOut length:(NSUInteger)dataOutMoved];
        result = [PQSecretTool base64EncodedStringWithData:data];
    }
    free(dataOut);
    return result;
}

@end
